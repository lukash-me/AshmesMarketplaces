...
public sealed class RuleSetsController : ControllerBase
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(PagedResponse<RuleSetListItemResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<PagedResponse<RuleSetListItemResponse>>> GetList([FromQuery] RuleSetListQuery query, CancellationToken cancellationToken)
    {
        var result = await _ruleSetService.GetListAsync(query, cancellationToken);
    ...
    public async Task<ActionResult<RuleSetResponse>> GetById(Guid id, CancellationToken cancellationToken)
    {
        var result = await _ruleSetService.GetByIdAsync(id, cancellationToken);
        return ToActionResult(result);
    }
    ...
    public async Task<ActionResult<RuleSetResponse>> Create([FromBody] CreateRuleSetRequest request, CancellationToken cancellationToken)
    {
        var result = await _ruleSetService.CreateAsync(request, cancellationToken);
        if (!result.IsSuccess)
            return ToActionResult(result);
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status409Conflict)]
    public async Task<ActionResult<RuleSetResponse>> Update(Guid id, [FromBody] UpdateRuleSetRequest request, CancellationToken cancellationToken)
    {
        var result = await _ruleSetService.UpdateAsync(id, request, cancellationToken);
        return ToActionResult(result);
    }
    ...
    {
        var result = await _ruleSetService.DeleteAsync(id, cancellationToken);
        if (result.IsSuccess)
            return NoContent();
