...
public sealed class RecommendationsController : ControllerBase
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(PagedResponse<RecommendationListItemResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<PagedResponse<RecommendationListItemResponse>>> GetList([FromQuery] RecommendationListQuery query, CancellationToken cancellationToken)
    {
        var result = await _recommendationService.GetListAsync(query, cancellationToken);
    ...
    {
        var result = await _recommendationService.GetByIdAsync(id, cancellationToken);
        return ToActionResult(result);
    }
    ...
    public async Task<ActionResult<RecommendationResponse>> Create([FromBody] CreateRecommendationRequest request, CancellationToken cancellationToken)
    {
        var result = await _recommendationService.CreateAsync(request, cancellationToken);
        if (!result.IsSuccess)
            return ToActionResult(result);
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status409Conflict)]
    public async Task<ActionResult<RecommendationResponse>> Update(Guid id, [FromBody] UpdateRecommendationRequest request, CancellationToken cancellationToken)
    {
        var result = await _recommendationService.UpdateAsync(id, request, cancellationToken);
        return ToActionResult(result);
    }
    ...
    {
        var result = await _recommendationService.DeleteAsync(id, cancellationToken);
        if (result.IsSuccess)
            return NoContent();
