...
public sealed class BrandsController : ControllerBase
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(PagedResponse<BrandListItemResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<PagedResponse<BrandListItemResponse>>> GetList(
        [FromQuery] ListQuery query,
        CancellationToken cancellationToken)
    {
        var result = await _brandService.GetListAsync(query, cancellationToken);
