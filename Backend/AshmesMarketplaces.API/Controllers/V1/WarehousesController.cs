...
public sealed class WarehousesController : ControllerBase
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(PagedResponse<WarehouseListItemResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<PagedResponse<WarehouseListItemResponse>>> GetList(
        [FromQuery] ListQuery query,
        CancellationToken cancellationToken)
    {
        var result = await _warehouseService.GetListAsync(query, cancellationToken);
